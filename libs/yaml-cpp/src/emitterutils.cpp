...
bool GetNextCodePointAndAdvance(int &codePoint, const char *&first,
                                const char *last) {
    if (first == last)
        return false;
...
bool IsValidPlainScalar(const char *str, std::size_t size,
                        FlowType::value flowType, bool allowOnlyAscii) {
    // check against null
...
bool WriteSingleQuotedString(ostream_wrapper &out, const char *str,
                             std::size_t size) {
    out << "'";
    int codePoint;
    for (const char *i = str;
...
bool WriteDoubleQuotedString(ostream_wrapper &out, const char *str,
    ...
    for (const char *i = str;
        ...
        default:
            ...
            } else if (codePoint == 0xFEFF) { // Byte order marks (ZWNS) should
                                              // be escaped (YAML 1.2, sec. 5.2)
                WriteDoubleQuoteEscapeSequence(out, codePoint, stringEscaping);
