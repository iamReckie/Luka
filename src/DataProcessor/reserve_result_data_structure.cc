...
void ReserveResultDataStructure::ConstructDataStructure(
  ...
  try {
    ...
    auto* table_context_ptr = data_helper->GetDataContext(L"Table");
    if (!table_context_ptr) {
      Logger::Log(L"Error: Failed to get TableData context\n");
      return;
    }
    const auto& table_data_map =
        std::any_cast<const TableDataStructure::TableDataMap&>(
            *table_context_ptr);
    ...
    const auto& insurance_results =
        std::any_cast<const InsuranceResultDataStructure::InsuranceResultList&>(
            *insurance_context_ptr);
    ...
    if (idx_tVn_1_loop == -1) idx_tVn_1_loop = 15;
    if (idx_tVn_2_loop == -1) idx_tVn_2_loop = 19;
    if (idx_NP_1 == -1) idx_NP_1 = 17;
    if (idx_Alpha_1 == -1) idx_Alpha_1 = 18;
    if (idx_NP_2 == -1) idx_NP_2 = 21;
    if (idx_Alpha_2 == -1) idx_Alpha_2 = 22;
    if (idx_tVn_1_end == -1) idx_tVn_1_end = 16;
    if (idx_tVn_2_end == -1) idx_tVn_2_end = 20;
    ...
    for (const auto& insurance_result : insurance_results) {
      ...
      for (int kk = 0; kk < nn; ++kk) {
          if (kk >= static_cast<int>(varTemp1.size())) break;
